var CopyQCore = (function () {
  ...
  function route(input) {
    ...
    var formats = input && input.formats || []; var eligible = normalizeFrequencyText(text) !== '';
    ...
    if (redaction.redacted) {
      var action = text.length >= 5000 ? 'big' : isStandaloneUrl(text) ? 'url' : isArtifact(redaction.text) ? 'artifacts' : isCode(redaction.text) ? 'code' : 'default';
      return { action: action, frequencyEligible: false, redactedText: redaction.text };
    }
