var CopyQCore = (function () {
  ...
  function credentialUrl(text) { return /^https?:\/\/[^/\s:@]+:[^/\s@]+@/i.test(text) || /[?&](?:access_token|api[_-]?key|password|secret|token)=[^&#\s]+/i.test(text); }
  ...
  function hashesFor(text) { text = String(text == null ? '' : text); return { version: 2, first: fnv1a(text), second: djb2(text), key: 'v2:' + fnv1a(text) + ':' + djb2(text) }; }
  ...
  function recordFrequency(state, legacy, text) {
    var next = { version: 2, counters: cloneCounters((state || {}).counters), clock: Number((state || {}).clock || 0) };
    var old = {}; var oldName; var keys; var key; var hash = hashesFor(text); var entry; var oldKey = legacyKiloHash(trim(text));
    for (oldName in legacy || {}) if (Object.prototype.hasOwnProperty.call(legacy, oldName)) old[oldName] = legacy[oldName];
    entry = next.counters[hash.key];
    if (!entry) { entry = { version: 2, first: hash.first, second: hash.second, count: Number(old[oldKey] || 0), lastUsed: 0 }; if (old[oldKey] !== undefined) delete old[oldKey]; }
    entry.count += 1; entry.lastUsed = ++next.clock; next.counters[hash.key] = entry;
    keys = []; for (key in next.counters) if (Object.prototype.hasOwnProperty.call(next.counters, key)) keys.push(key);
    keys.sort(function (a, b) { return (next.counters[a].lastUsed - next.counters[b].lastUsed) || (a < b ? -1 : 1); }); while (keys.length > 4096) delete next.counters[keys.shift()];
    return { state: next, legacy: old, result: { count: entry.count, promote: entry.count >= 6, hashes: hash } };
  }
  function route(input) { var text = String(input && input.text != null ? input.text : ''); var formats = input && input.formats || []; var result; if (hasFormat(formats, /^image\//i) || (input && input.mimeOwner && hasFormat(formats, new RegExp('^' + input.mimeOwner.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '$'))) || (input && input.mimeHidden && hasFormat(formats, new RegExp('^' + input.mimeHidden.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '$')))) return { action: 'passthrough' }; if (isHighConfidenceSecret(text, formats)) return { action: 'ignored' }; if (/^https?:\/\/\S+$/i.test(text)) return { action: 'passthrough' }; if (text.length >= 5000) return { action: 'big' }; if (isCode(text)) return { action: 'code' }; if (input && input.state) { result = recordFrequency(input.state, input.legacy || {}, text); return { action: result.result.promote ? 'frequent' : 'default', frequency: result }; } return { action: 'default' }; }
  ...
  return { isHighConfidenceSecret: isHighConfidenceSecret, isCode: isCode, isMarkdown: isMarkdown, hashesFor: hashesFor, legacyKiloHash: legacyKiloHash, recordFrequency: recordFrequency, route: route, sortFrequent: sortFrequent, titleCase: titleCase, sanitizeHtml: sanitizeHtml, toggleCase: toggleCase, validateJsonImport: validateJsonImport, validateRegex: validateRegex, runDependency: runDependency };
}());
